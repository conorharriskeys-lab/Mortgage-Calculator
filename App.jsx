...
function Slider({ label, value, min, max, step, onChange, display, color, prefix, suffix, inputType = "number" }) {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.35rem" }}>
        ...
        {editing ? (
          <div style={{ display: "flex", alignItems: "center", gap: "0.2rem" }}>
            {prefix && <span style={{ color: color || "#b5892a", fontSize: "0.8rem", fontFamily: "'DM Mono', monospace" }}>{prefix}</span>}
            <input
              autoFocus
              value={raw}
              ...
              onBlur={commitEdit}
              onKeyDown={handleKey}
              style={{ background: "#1e1e1e", border: `1px solid ${color || "#b5892a"}`, color: color || "#e8d5b0", fontSize: "0.88rem", fontFamily: "'Playfair Display', serif", fontWeight: 700, padding: "0.1rem 0.4rem", outline: "none", width: "90px", textAlign: "right", borderRadius: "2px" }}
            />
            {suffix && <span style={{ color: color || "#b5892a", fontSize: "0.8rem", fontFamily: "'DM Mono', monospace" }}>{suffix}</span>}
          </div>
        ) : (
          <span
            onClick={startEdit}
            title="Click to type a value"
            style={{ color: color || "#e8d5b0", fontSize: "0.92rem", fontWeight: "700", fontFamily: "'Playfair Display', serif", cursor: "text", borderBottom: `1px dashed ${color || "#b5892a"}44`, paddingBottom: "1px", transition: "border-color 0.2s" }}
          >{display}</span>
        )}
      </div>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.4rem" }}>
              <div style={{ background: "#161616", border: `1px solid ${color}20`, borderRadius: "4px", padding: "1.5rem" }}>
                <Slider label="Home Price" value={s.price} min={100000} max={2000000} step={5000} onChange={v => update(activeTab, "price", v)} display={fmt(s.price)} color={color} prefix="$" />
                <Slider label="Down Payment" value={s.downPct} min={5} max={50} step={0.5} onChange={v => update(activeTab, "downPct", v)} display={`${s.downPct}% · ${fmt(r.down)}`} color={color} suffix="%" />
                <Slider label="Interest Rate" value={s.rate} min={0.5} max={12} step={0.05} onChange={v => update(activeTab, "rate", v)} display={`${s.rate.toFixed(2)}%`} color={color} suffix="%" />
                <Slider label="Amortization" value={s.amort} min={5} max={30} step={1} onChange={v => update(activeTab, "amort", v)} display={`${s.amort} yrs`} color={color} suffix=" yrs" />
                <div style={{ color: "#2e2e2e", fontSize: "0.62rem", letterSpacing: "0.05em", marginTop: "-0.6rem", marginBottom: "0.8rem" }}>↑ click any value to type directly</div>
